...
class GoogleAPI:
	def google_apikey_extract(apk):
		print("[+] Google API Key Extractor is Starting...")
		apikey_search = f'strings {apk} | grep -iE "AIza[0-9A-Za-z\-_]"'
		google_api_key = Popen(apikey_search, shell=True, stdout=PIPE, stderr=STDOUT).communicate()[0]
		google_api_key = google_api_key.decode('utf-8').strip()
		if google_api_key == "":
			return False
		else:
			return google_api_key.replace("'", "")

class GoogleAPIKeyVulnScan:
